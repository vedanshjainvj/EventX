...
const Login = () => {
  ...
  <div className="mx-auto max-w-screen-2xl px-4 md:px-8">
    <h2 className="mb-4 text-center text-2xl font-bold  md:mb-8 lg:text-3xl">Login</h2>

    <form className="mx-auto max-w-lg rounded-lg login-form" method='post' onSubmit={loginData} >
      <div className="flex flex-col gap-4 p-4 md:p-8">
        <div>
          <label for="email" className="mb-2 inline-block text-sm  sm:text-base">Email</label>
          <input value={email} onChange={(e) => {
            setEmail(e.target.value)
          }} name="email" className="w-full rounded border bg-gray-50 px-3 py-2 text-gray-800 outline-none ring-indigo-300 transition duration-100 focus:ring" />
        </div>
        <div>
          <label for="password" className="mb-2 inline-block text-sm  sm:text-base">Password</label>
          <input type='password' name="password" value={pass} onChange={(e) => {
            console.log(e.target.value)
            setPass(e.target.value)}}
           className="w-full rounded border bg-gray-50 text-black px-3 py-2  outline-none ring-indigo-300 transition duration-100 focus:ring" />
        </div>
        ...
        <div className="relative flex items-center justify-center">
          <span className="relative  px-4 text-sm ">Log in with social</span>
        </div>
      ...
      <div className="flex items-center justify-center bg-slate-900 p-4">
        <p className="text-center text-sm ">Don't have an account? <a href="#" className="text-blue-500 transition duration-100 hover:text-indigo-600 active:text-indigo-700">Register</a></p>
      </div>
    </form>
  </div>
</div>
